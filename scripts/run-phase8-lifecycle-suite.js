/**
 * PHASE 8 — LIFECYCLE GAP ACCEPTANCE ORCHESTRATOR
 * Executes:
 * 1. Focused Track Lifecycle Verification (scripts/verify_phase8_lifecycle.py)
 * 2. Full Phase 8 Acceptance Verification (scripts/verify-phase8-tracking.js)
 * 3. Full Phase 7 Analytics Regression Verification (scripts/verify-phase7-analytics.js)
 */
...
async function main() {
  ...
  console.log('\n--- STEP 1: Running Focused Track Lifecycle Verification ---\n');
  try {
    execSync(`"${pythonExe}" scripts/verify_phase8_lifecycle.py`, { stdio: 'inherit' });
    console.log('\n>>> STEP 1: TRACK LIFECYCLE VERIFICATION PASSED <<<\n');
  } catch (err) {
    console.error('\n>>> STEP 1: TRACK LIFECYCLE VERIFICATION FAILED <<<\n');
    cleanup();
    process.exit(1);
  }

  console.log('\n--- STEP 2: Running Phase 8 Object Tracking Acceptance Suite ---\n');
  try {
    execSync('node scripts/verify-phase8-tracking.js', { stdio: 'inherit' });
    console.log('\n>>> STEP 2: PHASE 8 ACCEPTANCE PASSED <<<\n');
  } catch (err) {
    console.error('\n>>> STEP 2: PHASE 8 ACCEPTANCE FAILED <<<\n');
    cleanup();
    process.exit(1);
  }

  console.log('\n--- STEP 3: Running Phase 7 Analytics Regression Suite ---\n');
  try {
    execSync('node scripts/verify-phase7-analytics.js', { stdio: 'inherit' });
    console.log('\n>>> STEP 3: PHASE 7 REGRESSION PASSED <<<\n');
  } catch (err) {
    console.error('\n>>> STEP 3: PHASE 7 REGRESSION FAILED <<<\n');
    cleanup();
    process.exit(1);
  }
  ...
  cleanup();
  console.log('\n===============================================================');
  console.log('ALL PHASE 8 LIFECYCLE & REGRESSION SUITES PASSED (100% GREEN)');
  console.log('===============================================================');
  process.exit(0);
}
